...
class ModularProvider extends ServiceProvider
{
    ...
    public function getWebRoutes($mod, $sub, $relativePath, $path) 
    {
            ...
            if($mod != config('modular.groupWithoutPrefix')) {
                Route::group(
                    [
                        ...
                        'moddleware' => $this->getMiddleware($mod)
                    ],
    ...
    public function getApiRoutes($mod, $sub, $relativePath, $path) 
    {
        ...
        if(file_exists($routesPath)) {
            Route::group(
                [
                    'prefix' => strtolower($mod),
                    'moddleware' => $this->getMiddleware($mod, 'api')
                ],
                function() use ($mod, $sub, $routesPath) {
                    Route::namespace("App\Modules\\$mod\\$sub\Controllers")->
                            group($routesPath);
    ...
    public function getMiddleware($mod, $key = 'web') 
    {
        $middleware = [];
